# ArpSID changelog archive — v0.0.674 and earlier

Historical per-pass closure notes (including the v0.0.605 release-candidate
series and assorted PASS notes) moved out of `README.md` for hygiene
(audit P2 #56). Newest entries live in `README.md`; this file is append-only history.

---

# ArpSID v0.0.605 pass83 full-preflight and stale 128 wording closure note

## 0.0.641-pass281 — Header/comment merge compile closure

Pass281 continues the pass280 compile repair by auditing source/include comments for separator lines that accidentally swallowed real code after `// ----` breadcrumbs. It restores active code in SID runtime/envelope/parameter headers by splitting comment separators from declarations/statements, preserving the cleaned technical comments while preventing Clang/ObjC++ compile breakage and silent code removal.
...
# ArpSID 0.0.605 Release Candidate — audit closure build

This package is the v605 audit-closure source tree with the pass-1 through pass-6 fixes applied.

Canonical version sources now agree:

- `CMakeLists.txt`: `project(ArpSID VERSION 0.0.605 ...)`
- `VERSION.txt`: `0.0.605`
- `include/arpsid/version.h`: `ARPSID_PLUGIN_VERSION "0.0.605"`

## What is closed in this source package

- C64 PSID/RSID exactness diagnostics, strict/compatible RSID behaviour, full C64Platform rollback on failed PSID play calls, and multi-SID dropped-write exactness downgrade.
- SID-808 fixed-voice authority, waveform-bit mapping, ring/sync/filter routing, kit-state migration, and sample-offset drum scheduling.
- DR-SID canonical register-program wavetable runner wired into `DrSidEngine` as the authentic playback authority, with overlay mode remaining explicit for AnalogX0X8.
- KIT v2 state blob migration from the old v560/v1 1388-byte layout to the v2 1676-byte layout.
- KIT and DIGI render-block step-edge sample-offset propagation through a shared timing helper.
- DIGI current voice telemetry and smarter energy/near-end voice stealing.
- MIX FX processor sync from freshly-published GUI realtime state.
- Deterministic built-in C64 ROM vector mini-surface instead of undefined reset/IRQ bytes when external ROMs are not supplied.

## Validation in this environment

Linux/GCC CMake configure passes. The impacted production-path targets and regression tests build and pass in the sandbox. macOS AUv2/AUv3/Logic validation must still be run on macOS because this Linux sandbox has no Apple SDK, AUVal, or Logic.

## Notes

The old tab-architecture term “scaffold” remains only as the name of a compile-time enum state used to assert there are zero unfinished tabs; no shipped tab is marked with that status.
## PASS192 DIGI render queue closure

PASS192 fixes the final private `$D418` render-drain bug: the isolated DIGI bridge renderer no longer increments the timed-write index twice and therefore no longer skips every second accepted `$D418` event. The DIGI regression slice passes 18/18 tests. See `PASS192_DIGI_RENDER_QUEUE_CLOSURE.md`.



## PASS265 GUI/HUD polish

See `PASS265_GUI_HUD_EYECANDY_FINAL_POLISH.md` for the final no-FULL-button GUI/HUD eyecandy closure.

## PASS266 GUI/HUD Telemetry Polish

PASS266 keeps the custom FULL/fullscreen HUD button removed, changes zero-length DIGI length labels from `FULL` to `ALL`, and adds DC/bias plus headroom/clipping telemetry to the GPU/layer DIGI sample preview. PASS273/PASS274 tighten the preview contract: pending TAKE is quantized through `digiFloatToD418Nibble(...)` before drawing, and committed samples render from canonical 8 kHz `$D418` nibbles around silence midpoint 0x08.


### PASS267 GUI/HUD final closure
PASS267 removes remaining visible FULL wording from custom HUD labels, adds status-text fingerprinting so DIGI preview labels refresh when status changes without waveform changes, and publishes tooltip/accessibility telemetry for the GPU/layer sample preview.

## PASS268 — GUI / Tabs Cleanup Closure

PASS268 removes the obsolete hard-coded 11-segment tab-bar bootstrap and routes all tab labels/tooltips/segment counts through `ArpSIDConfigureVisibleTabBar_v268(...)`, using the canonical visible-tab arrays as the single source of truth. This removes the transient duplicate `DRSID` bootstrap label, keeps the hidden legacy SID Projection tab out of the visible ring, and makes prev/next tab navigation nil-safe for embedded host presentation.

- PASS269: Final GUI tab polish; canonical single visible-tab ring, compile-time tab invariants, tab accessibility summary, deterministic segment widths.

## PASS270 DIGI realtime / NaN / clamp closure

PASS270 closes the pass269 DIGI realtime audit findings: finite-safe legacy DIGI quantization/mixing, finite-safe AUTH `$D418` additive mix, finite-safe `$D418` stream quantizer, O(1) realtime user-sample lookup, defensive DIGI block-size guard, and AudioQueue fail-closed behavior on invalid route/format callbacks.

## PASS271 DIGI realtime finite-hardening closure

PASS271 closes follow-up finite-safety edges after PASS270: NaN/Inf sample positions are sanitized before clamp/cast, legacy and AUTH DIGI voice advance fails closed on invalid position/increment state, REC/MON meters use finite-safe publication helpers, output-tap preview sanitizes incoming mono frames before telemetry accumulation, and a regression test proves DIGI additive mixing cleans NaN/Inf pre-existing bus values.


## PASS272 AppKit tag compile closure

Removed invalid assignment to readonly `NSView.tag` on the DIGI sample preview view. Preview identity is tracked through `_digiSamplePreviewView_v262_`.

## PASS273 — Release 4-bit DIGI Preview Closure

- Final release cleanup based on pass272.
- Pending REC TAKE waveform is now visualized through the same 4-bit `$D418` nibble ladder used by commit/export.
- Committed sample preview converts stored `pcm8` back through `digiPcm8ToD418Nibble(...)` before drawing; no raw `clip.pcm[i] & 0x0F` preview path remains.
- Custom FULL/fullscreen HUD button remains removed.
- Preview remains GUI-thread CoreAnimation/Metal-composited only; no audio-thread GPU work.

## PASS274 — Canonical $D418 Technical Documentation and Comment Cleanup

PASS274 adds `TECHNICAL_D418_CANONICAL_4BIT_NIBBLE_SPEC.md`, a detailed release contract for the DIGI REC/import/preview/storage/playback pipeline. It documents canonical 8 kHz 4-bit `$D418` nibbles, why preview must be 4-bit visual rather than raw PCM, how pending TAKE is quantized for display, how committed clips recover nibbles with `digiPcm8ToD418Nibble(...)`, and what realtime/NaN/clamp rules are required. Stale comments from the old GPU-preview/tag transition were updated or removed.


## PASS275 source cleanup

The shipped source comments have been cleaned so they describe stable technical contracts rather than historical `PASS###` patch breadcrumbs. Older pass progression notes are archived under `docs/pass-history/`; the current `$D418` format contract remains in `TECHNICAL_D418_CANONICAL_4BIT_NIBBLE_SPEC.md`.

## 0.0.636-pass276 — GUI error/layout/tag cleanup

- Surfaces AU-bridge disconnect errors in HUDs instead of leaving stale UI state.
- Reports DIGI sample-bank/model corruption repairs to the DIGI HUD.
- Adds factory-slot duplicate postcondition and MIX/KIT/DIGI tag-range assertions.
- Hardens delayed drum audition note-off against controller teardown.
- Adds dynamic panel-inset helpers for less brittle manual layouts.



## PASS277 Resource Lifetime / Restore Closure

- Explicit CVDisplayLink release and callback-context clearing.
- Metal/MTKView teardown clears delegate and GPU resource references.
- DIGI AudioQueue/process-tap/REC/MON resources are stopped on editor close/dealloc.
- Delayed audition/display/observer blocks use teardown-aware weak/strong patterns.
- KIT/MIX/DIGI adapter pulls sanitize and republish repaired state to close restore round-trip.


## PASS278 Resource Lifetime / Restore Closure

Adds final block-observer token cleanup, teardown-gated delayed callbacks, and KIT/MIX/DIGI adapter round-trip verification after restore repair.

### PASS279 resource lifetime final release

- Canonical popout observer cleanup helper for block-based fullscreen observers.
- Popout close path now tears down observer tokens, Metal view, and content view consistently.
- CVDisplayLink create/start failures surface in HUD instead of leaving stale UI polling.
- Cross-thread MIDI HUD update uses weak/strong capture and teardown gating.



## PASS280 math_utils header compile closure

PASS280 fixes a macOS/Clang compile blocker in `include/arpsid/core/math_utils.h` where separator comments had swallowed several inline function declarations. The header now has clean standalone C++17 declarations and compiles before AUv2 source compilation proceeds.

### PASS282 — Header / ObjC++ comment-merge final compile closure

PASS282 restores declarations/statements that had been accidentally merged onto comment lines during earlier source cleanup. It specifically fixes DIGI atomic adapter method declarations, runtime DIGI control declarations, output-tap weak capture, AQ capture stats fields, and ObjC++ method definitions in the AU state-restore path.

## PASS283 source comment/merge guard

PASS283 audits PASS281/PASS282 and adds a source-level regression guard for the underlying failure class where separator comments accidentally share a physical line with declarations or statements. This prevents code such as C++ inline functions or Objective-C++ method declarations from being swallowed by `//` comments during future cleanup passes.



## PASS284 final source-root cleanup

PASS284 keeps the release root clean: historical PASS notes and old patch/diff artifacts live under `docs/pass-history/`, while root-level release files remain focused on build/install/use.  It also adds `release_root_clean_v705_tests.cpp` to prevent patch/history artifacts from drifting back into the shipped root.

### 0.0.645-pass285 — Release dead-code/artifact cleanup

- Removed stale pass-numbered script entrypoints.
- Removed historical pass/audit artifacts from release root.
- Kept active audit data under `docs/audit/`.
- `build.sh --install-auv2` delegates to the canonical AUv2 installer.
- Added release cleanup notes in `docs/RELEASE_DEAD_CODE_AND_ARTIFACT_CLEANUP.md`.


### 0.0.646-pass286 — C64/SIDCORE $D418 integration closure

- Makes the internal C64/SID write bus explicitly preserve every physical `$D418` write, including repeated identical volume samples, with absolute PHI2 timestamps.
- Adds `include/arpsid/core/c64_d418_capture.h` for extracting and zero-order-hold reconstructing `$D418` DIGI PCM from captured SID bus writes.
- Adds C64/SID bridge telemetry for `$D418` write count, repeated-value count, last value and last PHI2 cycle.
- Extends open-bus persistence to the documented C64-style window (`0x1D00` PHI2) and keeps color-RAM upper-nibble open-bus behavior intact.
- Updates VIC `$D019/$D01A` semantics: bits 4-6 read high, write-one-to-ack IRQ bits, and master IRQ bit follows enabled latched IRQs.
- Replaces the bare RTI-only deterministic KERNAL surface with a PSID-safe HLE vector/ACK surface: `$FF48` CINV IRQ chain, `$FE43` NMINV chain, `$EA31` CIA1 ACK, `$FE47` CIA2 ACK, and `$FF8D` RESTOR loop.
- Adds C64/SIDCORE regression tests for `$D418` capture, open bus persistence, VIC IRQ ACK/readback, and HLE vectors.

## PASS288 — C64/SIDCORE complete integration

Completes the concrete C64/SIDCORE integration surface after PASS287: exact 6510 `$0001` readback, unified processor-port semantics between PHI2 memory matrix and C64Platform, normal CPU-bus CIA2 `$DD00/$DD02` VIC bank switching, physical `$D418` mirror/repeated-write capture through the C64Platform path, and new `c64_sidcore_complete_integration_v709` regression coverage.

### PASS293 C64/SIDCORE VIC raster-compare completion

- Fixed VIC-II 9-bit raster compare to avoid modulo-wrapping out-of-range compare values.
- PAL compare `$1FF` now correctly never matches instead of falsely firing at line 199.
- Added `c64_vic_raster_compare_v716_tests` and CMake/CTest target.

## PASS294 C64/SIDCORE strict runtime guard

- Enforces the physical SID writable window in `MemoryMatrix` before PHI2 SID sink dispatch. `$D419-$D41F` no longer reach live SID write paths.
- Adds sink-level SID-hole write rejection in runtime/bridge paths, with exactness downgrade telemetry.
- Strict RSID no longer silently falls back to legacy instruction-atomic playback when PHI2 is unavailable.
- Aligns legacy decimal ADC/SBC N/Z flag policy with `Cpu6510Micro`.
- Adds `C64SidHoleWriteGateV717Tests` and `C64LegacyDecimalParityV718Tests`.

### Pass297 C64/SIDCORE strict refusal + CIA2 board-pin closure

- Strict RSID direct-cycle refusal now sets `StrictRsidNotPhi2` without falsely claiming `LegacyCpuPlayback`.
- CIA2 PA0/PA1 VIC-bank updates now use side-effect-free board pin state (`portAPins()`), not a generic CPU PRA read abstraction.
- Added regression tests `c64_strict_direct_phi2_refusal_v722_tests` and `c64_cia2_vic_bank_pins_v723_tests`.

### PASS300 C64/SIDCORE remaining findings telemetry closure

PASS300 adds explicit PSID-CIA compatibility-service telemetry and a consolidated guard for the remaining C64/SIDCORE audit findings. A PSID-CIA run through the legacy service is now counted and cannot be mistaken for strict/physical PHI2 execution. The new guard also verifies that strict RSID PHI2 path status remains separate from physical exactness when SID-read, HLE-ROM, CIA/VIC, or other ledger downgrades are present.
